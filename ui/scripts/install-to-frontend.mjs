...
// iteration on the body module we don't want to require a full frontend
...
// frontend registry can fetch /modules/yz-head.manifest.json and the Python
...
// Layout: satellites/yz-head/ui/scripts/ -> climb 4 levels to project root
const projectRoot = resolve(here, '..', '..', '..', '..')
const satelliteRoot = resolve(here, '..', '..')

const iifeSrc = resolve(here, '..', 'dist-lib', 'yz-head.iife.js')
const manifestSrc = resolve(satelliteRoot, 'manifest.json')

const iifeTargets = [
  resolve(projectRoot, 'frontend', 'public', 'modules', 'yz-head.iife.js'),
  resolve(projectRoot, 'backend', 'jarvyz', 'web', 'static', 'modules', 'yz-head.iife.js'),
]
const manifestTargets = [
  resolve(projectRoot, 'frontend', 'public', 'modules', 'yz-head.manifest.json'),
  resolve(projectRoot, 'backend', 'jarvyz', 'web', 'static', 'modules', 'yz-head.manifest.json'),
]
...
if (existsSync(manifestSrc)) {
  ...
  if (missing.length) {
    console.error(
      ...
      `Check satellites/yz-head/ui/src/index.ts.`,
    )
    process.exit(1)
  }
